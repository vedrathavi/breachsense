...
export function validateAgentOutput(payload) {
  ...
  // India
  if (!payload.legal.india || typeof payload.legal.india !== "object")
    return false;
  if (
    !Array.isArray(payload.legal.india.it_act) ||
    payload.legal.india.it_act.some((item) => !isNonEmptyString(item))
  )
    return false;
  if (
    !Array.isArray(payload.legal.india.dpdp) ||
    payload.legal.india.dpdp.some((item) => !isNonEmptyString(item))
  )
    return false;
  // International
  if (
    !payload.legal.international ||
    typeof payload.legal.international !== "object"
  )
    return false;
  // GDPR
  ...
  if (
    !gdpr ||
    typeof gdpr !== "object" ||
    typeof gdpr.applicable !== "boolean" ||
    !Array.isArray(gdpr.articles) ||
    typeof gdpr.penalty !== "string"
  )
    return false;
  // HIPAA
  ...
  if (
    !hipaa ||
    typeof hipaa !== "object" ||
    typeof hipaa.applicable !== "boolean" ||
    !Array.isArray(hipaa.provisions) ||
    typeof hipaa.penalty !== "string"
  )
    return false;
  // Others
